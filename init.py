...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 16 13:48:41 2019
...
def modelWithInit(init):
    ...
    model.add(Conv2D(80, (3, 3), padding='same', kernel_initializer = init, activation = 'relu'))
    model.add(Conv2D(80, (3, 3), padding='same', kernel_initializer = init, activation = 'relu')) model.add(Conv2D(80, (3, 3), padding='same', input_shape = (224, 224, 3), activation = 'relu'))
    model.add(Conv2D(80, (3, 3), padding='same', kernel_initializer = init, activation = 'relu'))
    model.add(MaxPooling2D(pool_size = (2, 2)))
    model.add(Dropout(0.25))
...
def training(model):
    ...
    train_generator = train_datagen.flow_from_directory('./train_set/', target_size = (224, 224), color_mode = "rgb", batch_size = 32,
                                                 class_mode = 'categorical', shuffle = True, seed = 2019);
                                                        
    valid_generator = test_datagen.flow_from_directory('./valid_set/', target_size = (224, 224), color_mode = "rgb", batch_size = 32, 
                                                       class_mode = 'categorical', shuffle = True, seed = 2019);
